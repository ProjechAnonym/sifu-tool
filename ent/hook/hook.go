...
// The DDNSFunc type is an adapter to allow the use of ordinary
// function as DDNS mutator.
type DDNSFunc func(context.Context, *ent.DDNSMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f DDNSFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.DDNSMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.DDNSMutation", m)
}
